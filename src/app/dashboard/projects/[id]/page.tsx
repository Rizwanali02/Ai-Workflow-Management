            ...
            <CardContent>
              <div className="flex items-center gap-4 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50">
                <Avatar className="w-8 h-8">
                  <AvatarImage src={project.managerId?.profileImg} alt={project.managerId?.name} className="object-cover" />
                  <AvatarFallback className="text-xs font-medium bg-slate-200 dark:bg-slate-700">
                    {project.managerId?.name?.charAt(0).toUpperCase()}
                  </AvatarFallback>
                </Avatar>
                <div>
                  ...
                  {project.members?.length > 0 ? (
                    project.members.map((member: any) => (
                      <div key={member._id} className="flex items-center gap-3">
                        <Avatar className="w-8 h-8">
                          <AvatarImage src={member.profileImg} alt={member.name} className="object-cover" />
                          <AvatarFallback className="text-xs font-medium bg-slate-200 dark:bg-slate-700">
